...
const Documents = ({documents}:Props) => {
  return <section className="max-w-7xl mx-auto px-8 py-10">
    ...
    <div className="mt-4 w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {documents.map((doc) => (
        <Link href={doc.src!} key={doc.id} className="w-full overflow-hidden border border-accent rounded-md shadow aspect-square flex flex-col items-center justify-center">
          <DocumentIcon className="h-20 w-20 md:h-24 md:w-24 text-accent" />
