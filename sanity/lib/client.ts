...
export async function getHomepageHero(): Promise<HomepageHero> {
  const homepageHero = await client.fetch(
    `*[_type == "homepageHero"]{heading, subtitle, image}[0]`
  );
  return homepageHero;
}
...
export async function getHomepageContent(): Promise<HomepageContent> {
  const homepageContent = await client.fetch(
    `*[_type == "homepageContent"]{objectiveTitle, objectiveDescription, homepageStory, storyContent}[0]`
  );
  return homepageContent;
}
export async function getHomepageTimeline(): Promise<HomepageTimeline[]> {
  const homepageTimeline = await client.fetch(
    `*[_type == "homepageTimeline"]{Year, timelineDescription}|order(Year asc)`
  );
  return homepageTimeline;
}

export async function getNMBHero(): Promise<NmbPageHero> {
  const nmbHero = await client.fetch(
    `*[_type == "nmbpageHero"]{nmbheading, nmbSubtitle, nmbHeroImage}[0]`
  );
  return nmbHero;
}
...
export async function getNmbPageDescription(): Promise<NmbPageDescription> {
  const nmbDescription = await client.fetch(
    `*[_type == "nmbDescription"]{descriptionText, content}[0]`
  );
...
export async function nmbPageFocus(): Promise<NmbPageFocus> {
  const nmbFocus = await client.fetch(
    `*[_type == "nmbfocus"]{nmbfocuHeading, nmbfocusSubheading, nmbFocusList, nmbfocusDescription}[0]`
  );
  return nmbFocus;
}
...
export async function getNMBPageFunders(): Promise<NMBPageFunders[]> {
  const nmbFunders = await client.fetch(
    `*[_type == "nmbFunders"]{funderName, funderWebsite, nmbfunderLogo}`
  );
  return nmbFunders;
}
...
export async function getNationalPageHero(): Promise<NationalPageHero> {
  const nationalHero = await client.fetch(
    `*[_type == "nationalpageHero"]{heading, subtitle, image, subtitleContent}[0]`
  );
  return nationalHero;
}
...
export async function getNationalPageDescription(): Promise<NationalPageDescription> {
  const nationalDescription = await client.fetch(
    `*[_type == "nationalDescription"]{descriptionText}[0]`
  );
...
export async function getProvinceDescription(): Promise<ProvinceDescription> {
  const provinceDescription = await client.fetch(
    `*[_type == "provinceDescription"]{descriptionText}[0]`
  );
...
export async function getNationalPageFocus(): Promise<NationalPageFocus> {
  const nationalFocus = await client.fetch(
    `*[_type == "nationalfocus"]{focusHeading, focusSubheading, nationalList, nationFocusDescription}[0]`
  );
...
export async function getNationalPageRolePlayers(): Promise<
  NationalPageRolePlayers[]
> {
  const nationalRolePlayers = await client.fetch(
    `*[_type == "roleplayers"]{title, description}`
  );
...
export async function getNationalPagePartners(): Promise<
  NationalPagePartners[]
> {
  const nationalPartners = await client.fetch(
    `*[_type == "nationalPartners"]{partnerName, partnerWebsite, partnerLogo}`
  );
...
export async function getNisspHero(): Promise<NisspHero> {
  const nisspHero = await client.fetch(
    `*[_type == "nisspPageHero"]{heading, subtitle, image}[0]`
  );
  return nisspHero;
}
...
export async function getNisspDescription(): Promise<NisspDescription> {
  const nisspDescription = await client.fetch(
    `*[_type == "niispdescription"]{descriptionText}[0]`
  );
...
export async function getPartners(): Promise<any[]> {
  const partners =
    await client.fetch(`*[_type == "partnerProfiles"] | order(organisationName asc){
    organisationName,
    description,
    logo,
    slug,
...
export async function getPageListGroupBySlug(slug: string) {
  const pageListGroup = await client.fetch(
    `*[_type == "page_list_group" && page->slug.current == "${slug}"]{
      _id,
      list_title,
      list_subtitle,
      list_items,
